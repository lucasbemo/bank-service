...
export const options = {
  ...
  thresholds: {
    ...
    http_req_duration: [{ threshold: "p(95)<400", abortOnFail: false }],
  },
};
...
export default function (data) {
  ...
  const idempotencyKey = `smoke-${data.runId}-${__VU}-${__ITER}`;
...
export function handleSummary(data) {
  const summary = responseSummary(data, "smoke", baseUrl);
  return {
    "tests/reports/smoke_summary.json": JSON.stringify(summary, null, 2),
  };
}
