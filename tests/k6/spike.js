...
export default function (data) {
  ...
  const idempotencyKey = `spike-${data.runId}-${__VU}-${__ITER}`;
...
export function handleSummary(data) {
  const summary = responseSummary(data, "spike", baseUrl);
  return {
    "tests/reports/spike_summary.json": JSON.stringify(summary, null, 2),
  };
}
